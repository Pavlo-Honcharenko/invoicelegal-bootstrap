(() => {
    ...
    (function(TheDatepicker) {
        ...
        var ClassNames = function() {
            ...
            ClassNames.prototype.normalizeClassName_ = function(className) {
                ...
                if (typeof className !== "object" || className.constructor !== Array) className = TheDatepicker.Helper_.checkString_(parameterName, className).split(/\s+/);
                var result = [];
                for (var index = 0; index < className.length; index++) {
                    var name_1 = TheDatepicker.Helper_.checkString_(parameterName, className[index]);
                    if (name_1) result.push(name_1);
                }
                return result;
            };
            return ClassNames;
    ...
    (function(TheDatepicker) {
        ...
        var ParsedDateData = function() {
            ...
            ParsedDateData.prototype.createDate = function() {
                if (this.day === null || this.month === null || this.year === null) throw new CannotParseDateException;
                var date = new Date(this.year, this.month - 1, this.day);
                if (isNaN(date.getTime())) throw new CannotParseDateException;
                while (date.getDate() !== this.day || date.getMonth() !== this.month - 1 || date.getFullYear() !== this.year) if (this.day > 28) {
                    this.day--;
                    date = new Date(this.year, this.month - 1, this.day);
                } else throw new CannotParseDateException;
                return date;
            };
        ...
        var DateConverter_ = function() {
            ...
            DateConverter_.parseDate_ = function(text, options) {
                if ((text.trim ? text.trim() : text) === "") return null;
                ...
                for (var position = 0; position < format.length; position++) {
                    var char = format.substring(position, position + 1);
                    if (escapeNext) escapeNext = false; else if (char === DateConverter_.escapeChar_) {
                        escapeNext = true;
                        continue;
                    } else {
                        ...
                        if (parser) {
                            try {
                                ...
                                if (!(error instanceof CannotParseDateException)) throw error;
            ...
            DateConverter_.isValidChar_ = function(textChar, options) {
                if (textChar === "" || /[0-9-]/.test(textChar)) return true;
                var format = options.getInputFormat();
                var escapeNext = false;
                for (var position = 0; position < format.length; position++) {
                    var char = format.substring(position, position + 1);
                    if (escapeNext) escapeNext = false; else if (char === DateConverter_.escapeChar_) {
                        ...
                        if (phrases) {
                            ...
                            for (var index = 0; index < phrases.length; index++) if (phrases[index].toLowerCase().indexOf(textCharLower) > -1) return true;
                            continue;
                        }
                    }
                    if (textChar === char) return true;
                }
                return false;
            };
            ...
            DateConverter_.parseDay_ = function(text, dateData) {
                ...
                if (!/[12][0-9]|3[01]/.test(day)) {
                    ...
                    if (!/[1-9]/.test(day)) throw new CannotParseDateException;
                }
            ...
            DateConverter_.parseDayOfWeekByTranslator_ = function(text, translate) {
                ...
                for (var dayOfWeek = 0; dayOfWeek < 7; dayOfWeek++) {
                    ...
                    if (text.substring(0, translation.length).toLowerCase() === translation.toLowerCase() && translation.length > matchedTranslationLength) matchedTranslationLength = translation.length;
                }
                if (matchedTranslationLength > 0) return matchedTranslationLength;
                var took = 0;
                while (/[a-zA-Z]/.test(text.substring(0, 1))) {
                    text = text.substring(1);
                    took++;
                    if (took === maxLength) break;
                }
                return took;
            };
            DateConverter_.parseMonth_ = function(text, dateData) {
                ...
                if (month !== "10" && month !== "11" && month !== "12") {
                    ...
                    if (!/[1-9]/.test(month)) throw new CannotParseDateException;
                }
            ...
            DateConverter_.parseMonthByTranslator_ = function(text, dateData, translate) {
                ...
                if (matchedMonth === null) throw new CannotParseDateException;
            ...
            DateConverter_.parseYear_ = function(text, dateData, options) {
                ...
                while (/[0-9]/.test(text.substring(yearLength, yearLength + 1))) {
                    if (isNegative && yearLength + 1 > maxNegativeLength || !isNegative && yearLength + 1 > maxPositiveLength) break;
                    yearLength++;
                }
                if (yearLength === 0) throw new CannotParseDateException;
            ...
            DateConverter_.parseYearTwoDigits_ = function(text, dateData, options) {
                ...
                if (!/[0-9]{2}/.test(yearEnd)) throw new CannotParseDateException;
    ...
    (function(TheDatepicker) {
        ...
        var Datepicker = function() {
            function Datepicker(input, container, options) {
                ...
                if (!(this instanceof Datepicker)) throw new Error('Creation must be performed by "new" keyword.');
                if (input && !TheDatepicker.Helper_.isElement_(input)) throw new Error("Input was expected to be null or an HTMLElement.");
                if (container && !TheDatepicker.Helper_.isElement_(container)) throw new Error("Container was expected to be null or an HTMLElement.");
                if (!input && !container) throw new Error("At least one of input or container is mandatory.");
                if (options && !(options instanceof TheDatepicker.Options)) throw new Error("Options was expected to be an instance of Options");
                ...
                } else if (container.datepicker) throw new Error(duplicateError + "container.");
                ...
                if (input) {
                    if (input.datepicker) throw new Error(duplicateError + "input.");
            ...
            Datepicker.prototype.render = function() {
                ...
                switch (this.initializationPhase_) {
                  ...
                  case InitializationPhase.Waiting:
                    ...
                    if (!this.viewModel_.selectPossibleDate_()) this.updateInput_();
                    return;

                  case InitializationPhase.Untouched:
                    this.preselectFromInput_();
                    this.createDeselectElement_();
                    if (!this.viewModel_.selectInitialDate_(null)) this.updateInput_();
            ...
            Datepicker.prototype.open = function(event) {
                ...
                if (this.initializationPhase_ === InitializationPhase.Untouched) this.render();
                if (this.initializationPhase_ === InitializationPhase.Waiting) {
                    this.initializationPhase_ = InitializationPhase.Ready;
                    this.render();
                    Datepicker.hasClickedViewModel_ = true;
                }
                if (!Datepicker.activateViewModel_(event, this)) return false;
                if (this.inputClickable_) this.inputClickable_.focus();
                return true;
            };
            ...
            Datepicker.prototype.close = function(event) {
                if (event === void 0) event = null;
                if (!this.viewModel_.isActive_()) return true;
                if (!Datepicker.activateViewModel_(event, null)) return false;
                if (this.inputClickable_) this.inputClickable_.blur();
                return true;
            };
            ...
            Datepicker.prototype.destroy = function() {
                if (this.initializationPhase_ === InitializationPhase.Destroyed) return;
                for (var index = 0; index < this.listenerRemovers_.length; index++) this.listenerRemovers_[index]();
                this.listenerRemovers_ = [];
                if (this.isContainerExternal_) this.container.innerHTML = ""; else this.container.parentNode.removeChild(this.container);
            ...
            Datepicker.prototype.parseRawInput = function() {
                return this.inputText_ ? TheDatepicker.DateConverter_.parseDate_(this.inputText_.value, this.options) : null;
            };
            ...
            Datepicker.prototype.readInput_ = function(event) {
                ...
                try {
                    ...
                    if (!(error instanceof TheDatepicker.CannotParseDateException)) throw error;
                }
                return false;
            };
            Datepicker.prototype.updateInput_ = function() {
                if (!this.inputText_ || this.inputText_ === Datepicker.document_.activeElement) return;
            ...
            Datepicker.onDatepickerReady = function(element, callback) {
                ...
                if (!TheDatepicker.Helper_.isElement_(element)) throw new Error("Element was expected to be an HTMLElement.");
                callback = TheDatepicker.Helper_.checkFunction_("Callback", callback);
                var promise = null;
                var promiseResolve = null;
                if (typeof Promise !== "undefined") promise = new Promise((function(resolve) {
                ...
                if (element.datepicker && element.datepicker instanceof Datepicker) {
                    ...
                    if (promiseResolve) promiseResolve(element.datepicker);
                } else Datepicker.readyListeners_.push({
                    promiseResolve,
                    element,
                    callback
                });
            ...
            Datepicker.prototype.createDeselectElement_ = function() {
                var _this = this;
                if (!this.inputText_ || !this.options.isDeselectButtonShown() || this.deselectElement_) return null;
                ...
                if (title !== "") deselectButton.title = title;
            ...
            Datepicker.prototype.updateDeselectElement_ = function() {
                if (!this.deselectElement_) return;
            ...
            Datepicker.prototype.preselectFromInput_ = function() {
                if (this.inputText_) try {
                    var date = this.parseRawInput();
                    if (date) this.options.setInitialDate(date);
                } catch (error) {
                    if (!(error instanceof TheDatepicker.CannotParseDateException)) throw error;
                }
            };
            Datepicker.prototype.initListeners_ = function() {
                ...
                if (!Datepicker.areGlobalListenersInitialized_) {
                    var checkMiss = function(event) {
                        if (Datepicker.hasClickedViewModel_) Datepicker.hasClickedViewModel_ = false; else Datepicker.activateViewModel_(event, null);
                    };
                    ...
                    TheDatepicker.Helper_.addEventListener_(Datepicker.document_, TheDatepicker.ListenerType_.KeyDown, (function(event) {
                        if (Datepicker.activeViewModel_) Datepicker.activeViewModel_.triggerKeyPress_(event);
                    }));
                ...
                if (this.deselectButton_) {
                    var hitIfActive = function(event) {
                        if (_this.viewModel_.isActive_()) hit(event);
                    };
                ...
                if (this.inputText_) {
                    this.listenerRemovers_.push(TheDatepicker.Helper_.addEventListener_(this.inputText_, TheDatepicker.ListenerType_.KeyDown, (function(event) {
                        ...
                        if (event.keyCode === TheDatepicker.KeyCode_.Esc && _this.options.isClosedOnEscPress()) _this.close(event);
                    })));
                    ...
                    this.listenerRemovers_.push(TheDatepicker.Helper_.addEventListener_(this.inputText_, TheDatepicker.ListenerType_.KeyPress, (function(event) {
                        ...
                        if (charCode && !_this.canType_(String.fromCharCode(charCode))) TheDatepicker.Helper_.preventDefault_(event);
                    })));
                }
            };
            ...
            Datepicker.prototype.triggerReady_ = function(element) {
                for (var index = Datepicker.readyListeners_.length - 1; index >= 0; index--) {
                    ...
                    if (listener.element === element) {
                        ...
                        if (listener.promiseResolve) listener.promiseResolve(this);
                        Datepicker.readyListeners_.splice(index, 1);
                    }
                }
            };
            Datepicker.prototype.triggerReadyListener_ = function(callback, element) {
                if (callback) callback.call(element, this, element);
            };
            Datepicker.prototype.onActivate_ = function() {
                if (this.initializationPhase_ === InitializationPhase.Destroyed) return;
                this.updateContainer_();
                if (this.inputText_) this.inputText_.readOnly = !this.options.isKeyboardOnMobile() && TheDatepicker.Helper_.isMobile_();
            };
            Datepicker.prototype.updateContainer_ = function() {
                if (this.isContainerExternal_) return;
                this.container.className = "";
                TheDatepicker.HtmlHelper_.addClass_(this.container, TheDatepicker.ClassNameType.Container, this.options);
                if (this.options.isDarkModeEnabled()) TheDatepicker.HtmlHelper_.addClass_(this.container, TheDatepicker.ClassNameType.ContainerDarkMode, this.options);
                if (this.options.isFullScreenOnMobile()) TheDatepicker.HtmlHelper_.addClass_(this.container, TheDatepicker.ClassNameType.ContainerResponsive, this.options);
            ...
            Datepicker.setBodyClass_ = function(enable) {
                ...
                if (!hasClass && enable) body.className += (className.length > 0 ? " " : "") + pageClass; else if (hasClass && !enable) {
                    var search = pageClass;
                    if (className.indexOf(" " + pageClass) > -1) search = " " + pageClass; else if (className.indexOf(pageClass + " ") > -1) search = pageClass + " ";
            ...
            Datepicker.activateViewModel_ = function(event, datepicker) {
                ...
                if (activeViewModel === viewModel) return true;
                if (activeViewModel && !activeViewModel.setActive_(event, false)) return false;
                if (Datepicker.activeViewModel_ !== activeViewModel) return true;
                if (!viewModel) {
                    Datepicker.setBodyClass_(false);
                    Datepicker.activeViewModel_ = null;
                    return true;
                }
                if (!viewModel.setActive_(event, true)) return false;
                if (Datepicker.activeViewModel_ !== activeViewModel) return true;
    ...
    (function(TheDatepicker) {
        var Day = function() {
            ...
            Day.prototype.isEqualToDate = function(date) {
                return TheDatepicker.Helper_.isValidDate_(date) && this.dayNumber === date.getDate() && this.month === date.getMonth() + 1 && this.year === date.getFullYear();
            };
    ...
    (function(TheDatepicker) {
        ...
        })(TheDatepicker.DayOfWeek || (TheDatepicker.DayOfWeek = {}));
        (function(Month) {
        ...
        })(TheDatepicker.Month || (TheDatepicker.Month = {}));
        (function(Align) {
            Align[Align["Left"] = 1] = "Left";
            Align[Align["Right"] = 2] = "Right";
            Align[Align["Center"] = 3] = "Center";
        })(TheDatepicker.Align || (TheDatepicker.Align = {}));
        (function(Position) {
        ...
        })(TheDatepicker.Position || (TheDatepicker.Position = {}));
        (function(KeyCode_) {
        ...
        })(TheDatepicker.KeyCode_ || (TheDatepicker.KeyCode_ = {}));
        var ListenerType_;
        ...
        var Helper_ = function() {
            ...
            Helper_.normalizeDate_ = function(parameterName, value, isNullable, options) {
                if (!value) {
                    if (!isNullable) throw new Error(parameterName + " cannot be empty.");
                    return null;
                }
                if (value instanceof TheDatepicker.Day) return value.getDate();
                if (typeof value === "string") {
                    value = value.trim ? value.trim() : value;
                    if (value === "today" || value === "now") return options.getToday();
                    if (value === "tomorrow") {
                    ...
                    if (matches) {
                        ...
                        var month = void 0;
                        for (month = 0; month < Helper_.months_.length; month++) if (matches[2].toLowerCase() === Helper_.months_[month]) {
                            date.setMonth(month);
                            date.setDate(day);
                            break;
                        }
                    ...
                    while (parsedValue) {
                        ...
                        if (!parsedValue) return date;
                    }
                    date = Helper_.resetTime_(new Date(value));
                    if (!isNaN(date.getTime())) return date;
                } else if (Helper_.isValidDate_(value)) {
                    date = Helper_.resetTime_(new Date(value.getTime()));
                    if (!isNaN(date.getTime())) return date;
                }
                throw new Error(parameterName + " was expected to be a valid Date string or valid Date or TheDatepicker.Day" + (isNullable ? " or null." : "."));
            };
            ...
            Helper_.inArray_ = function(list, item) {
                for (var index = 0; index < list.length; index++) if (list[index] === item) return true;
                return false;
            };
            Helper_.addEventListener_ = function(element, listenerType, listener, isPassive) {
                ...
                if (element.addEventListener) {
                    var options = void 0;
                    if (isPassive && Helper_.isPassiveEventListenerSupported_()) options = {
                        passive: true
                    };
                ...
                element[listenerProperty] = function(event) {
                    ...
                    if (originalListener) originalListener.call(element, event);
                    listener(event);
                };
                return function() {
            ...
            Helper_.checkString_ = function(parameterName, value, checkNonEmpty) {
                ...
                if (!checkNonEmpty && !value) return "";
                if (typeof value !== "string" || checkNonEmpty && value === "") throw new Error(parameterName + " was expected to be a" + (checkNonEmpty ? " non empty" : "") + " string.");
                return value;
            };
            Helper_.checkNumber_ = function(parameterName, value, min, max) {
                ...
                if (typeof value !== "number" || isNaN(value) || min !== null && value < min || max !== null && value > max) throw new Error(parameterName + " was expected to be a valid number" + (min !== null ? " from " + min : "") + (max !== null ? " to " + max : "") + ".");
                return value;
            };
            Helper_.checkFunction_ = function(parameterName, value, isNullable) {
                ...
                if (typeof value !== "function") throw new Error(parameterName + " was expected to be a function" + (isNullable ? " or null" : "") + ".");
                return value;
            };
            Helper_.warnDeprecatedUsage_ = function(deprecatedMethod, alternateMethods) {
                if (!window.console) return;
                for (var index = 0; index < Helper_.deprecatedMethods_.length; index++) if (deprecatedMethod === Helper_.deprecatedMethods_[0]) return;
            ...
            Helper_.addSwipeListener_ = function(element, listener) {
                ...
                Helper_.addEventListener_(element, ListenerType_.TouchMove, (function(event) {
                    ...
                    if (Math.abs(diffX) > minDistance.x) moveDirection = diffX > 0 ? TheDatepicker.MoveDirection_.Right : TheDatepicker.MoveDirection_.Left; else if (Math.abs(diffY) > minDistance.x) moveDirection = diffY > 0 ? TheDatepicker.MoveDirection_.Down : TheDatepicker.MoveDirection_.Up;
                    if (moveDirection) {
            ...
            Helper_.months_ = [ "january", "february", "march", "april", "may", "june", "july", "august", "september", "october", "november", "december" ];
    ...
    (function(TheDatepicker) {
        var HtmlHelper_ = function() {
            ...
            HtmlHelper_.createTableHeader_ = function(cells, type, options) {
                ...
                for (var index = 0; index < cells.length; index++) row.appendChild(cells[index]);
            ...
            HtmlHelper_.createTableBody_ = function(rows, type, options) {
                ...
                for (var index = 0; index < rows.length; index++) tableBody.appendChild(rows[index]);
                return tableBody;
            };
            ...
            HtmlHelper_.createSelectInput_ = function(selectOptions, onChange, options) {
                ...
                for (var index = 0; index < selectOptions.length; index++) input.appendChild(HtmlHelper_.createSelectOption_(selectOptions[index].value, selectOptions[index].label));
            ...
            HtmlHelper_.addClass_ = function(element, type, options) {
                ...
                for (var index = 0; index < classNames.length; index++) classNames[index] = options.prefixClass_(classNames[index]);
    ...
    (function(TheDatepicker) {
        ...
        var Options = function() {
            ...
            Options.prototype.setDateAvailabilityResolver = function(resolver) {
                ...
                if (resolver) this.addDateAvailabilityResolver(resolver);
            };
            ...
            Options.prototype.getInitialMonth = function() {
                ...
                var initialMonth = primarySource ? new Date(primarySource.getTime()) : secondarySource ? new Date(secondarySource.getTime()) : this.getToday();
            ...
            Options.prototype.findPossibleAvailableDate = function(date) {
                if (this.isAllowedEmpty()) return date && this.isDateInValidity(date) && this.isDateAvailable(date) ? new Date(date.getTime()) : null;
            ...
            Options.prototype.calculateNearestAvailableDate_ = function(date, minDate, maxDate) {
                ...
                do {
                    if (increasedDate) {
                        ...
                        if (increasedDate.getTime() > maxTimestamp) increasedDate = null; else if (this.isDateAvailable(increasedDate)) return increasedDate;
                    }
                    if (decreasedDate) {
                        decreasedDate.setDate(decreasedDate.getDate() - 1);
                        if (decreasedDate.getTime() < minTimestamp) decreasedDate = null; else if (this.isDateAvailable(decreasedDate)) return decreasedDate;
                    }
                    maxLoops--;
            ...
            Options.prototype.getMinMonth_ = function() {
                ...
                if (!minMonth) minMonth = this.getMinDate_();
                return minMonth;
            };
            ...
            Options.prototype.isDateAvailable = function(date) {
                ...
                for (var index = 0; index < dateAvailabilityResolvers.length; index++) if (!dateAvailabilityResolvers[index](new Date(date.getTime()))) return false;
                return true;
            };
            Options.prototype.getCellContent = function(day) {
                if (this.cellContentResolver_) return this.cellContentResolver_(day);
            ...
            Options.prototype.getCellStructure_ = function() {
                if (this.cellContentStructureResolver_) return this.cellContentStructureResolver_.init();
                return TheDatepicker.HtmlHelper_.createSpan_();
            };
            Options.prototype.updateCellStructure_ = function(element, day) {
                if (this.cellContentStructureResolver_) this.cellContentStructureResolver_.update(element, day); else element.innerText = this.getCellContent(day);
            };
            ...
            Options.prototype.getCellClasses = function(day) {
                ...
                for (var index = 0; index < cellClassesResolvers.length; index++) {
                    ...
                    if (typeof classes === "string") result.push(classes); else if (typeof classes === "object" && classes.constructor === Array) result = result.concat(classes);
                }
                return result;
            };
            Options.prototype.modifyDay = function(day) {
                var dayModifiers = this.dayModifiers_.slice(0);
                for (var index = 0; index < dayModifiers.length; index++) dayModifiers[index](day);
            };
            ...
            Options.prototype.checkConstraints_ = function(minDate, maxDate) {
                if (minDate && maxDate && minDate.getTime() > maxDate.getTime()) throw new Error("Min date cannot be higher then max date.");
            };
            ...
            Options.prototype.removeCallback_ = function(callbacksList, parameterName, callback) {
                ...
                if (!callback) callbacksList.splice(0, callbacksList.length); else {
                    var callbacks = callbacksList.slice(0);
                    for (var index = callbacks.length - 1; index >= 0; index--) if (callbacks[index] === callback) callbacksList.splice(index, 1);
                }
            };
            ...
            Options.prototype.offEvent_ = function(eventType, listener) {
                ...
                if (!listener) this.listeners_[eventType] = []; else {
                    var newListeners = [];
                    for (var index = 0; index < this.listeners_[eventType].length; index++) if (this.listeners_[eventType][index] !== listener) newListeners.push(this.listeners_[eventType][index]);
            ...
            Options.prototype.triggerEvent_ = function(eventType, caller) {
                ...
                for (var index = 0; index < listeners.length; index++) if (caller(listeners[index]) === false) return false;
                return true;
            };
            return Options;
        }();
        ...
        var Template_ = function() {
            ...
            Template_.prototype.render_ = function(viewModel) {
                if (!this.mainElement_) {
                    if (this.hasInput_ && this.options_.isHiddenOnBlur() && !viewModel.isActive_()) return;
            ...
            Template_.prototype.updateMainElement_ = function(viewModel) {
                ...
                if (this.options_.isDarkModeEnabled()) TheDatepicker.HtmlHelper_.addClass_(this.mainElement_, TheDatepicker.ClassNameType.MainDarkMode, this.options_);
            };
            Template_.prototype.updateTableElements_ = function(viewModel) {
                ...
                if (this.tableOfYearsElement_) this.tableOfYearsElement_.style.display = viewModel.yearSelectionState_ ? "" : "none";
            };
                    ...
                    TheDatepicker.Helper_.addSwipeListener_(body, (function(event, moveDirection) {
                        ...
                        switch (moveDirection) {
                          ...
                          case TheDatepicker.MoveDirection_.Up:
                            if (_this.tableOfYearsElement_ && _this.options_.isTableOfYearsOnSwipeDownEnabled() && viewModel.canSetYearSelectionActive_(isForward)) change = function() {
                          ...
                          case TheDatepicker.MoveDirection_.Right:
                            if (_this.options_.isMonthChangeOnSwipeEnabled() && viewModel.canGoDirection_(isForward)) change = function() {
                                viewModel.goDirection_(event, isForward);
                            };
                        }
                        if (change) _this.slideTable_(viewModel, moveDirection, change);
                    }));
                }
            ...
            Template_.prototype.createHeaderElement_ = function(viewModel) {
                ...
                } else state.appendChild(this.createMonthAndYearElement_(viewModel));
            ...
            Template_.prototype.updateTopElement_ = function(viewModel) {
                var isVisible = this.options_.getTitle() !== "" || this.options_.isResetButtonShown() || this.hasInput_ && this.options_.isCloseButtonShown();
            ...
            Template_.prototype.createGoElement_ = function(viewModel, isForward) {
                ...
                var goButton = TheDatepicker.HtmlHelper_.createAnchor_((function(event) {
                    ...
                    if (viewModel.canGoDirection_(isForward)) _this.slideTable_(viewModel, moveDirection, (function() {
                        viewModel.goDirection_(event, isForward);
                    }));
                }), this.options_);
                goButton.innerHTML = isForward ? this.options_.getGoForwardHtml() : this.options_.getGoBackHtml();
                goElement.appendChild(goButton);
                if (isForward) this.goForwardElement_ = goButton; else this.goBackElement_ = goButton;
                return goElement;
            };
            Template_.prototype.updateGoElement_ = function(viewModel, isForward) {
                ...
                this.updateTitle_(goElement, viewModel.yearSelectionState_ ? isForward ? TheDatepicker.TitleName.GoForwardTableOfYears : TheDatepicker.TitleName.GoBackTableOfYears : isForward ? TheDatepicker.TitleName.GoForward : TheDatepicker.TitleName.GoBack);
            };
            Template_.prototype.createMonthElement_ = function(viewModel) {
                var _this = this;
                var options = [];
                for (var monthNumber = 0; monthNumber < 12; monthNumber++) options.push({
                    value: monthNumber + "",
                ...
                var selectElement = TheDatepicker.HtmlHelper_.createSelectInput_(options, (function(event, monthNumber) {
                    ...
                    if (!viewModel.goToMonth_(event, newMonth)) _this.monthSelect_.value = currentMonth.getMonth() + "";
                }), this.options_);
            ...
            Template_.prototype.createYearElement_ = function(viewModel) {
                ...
                } else yearActiveElement = TheDatepicker.HtmlHelper_.createSelectInput_([], (function(event, year) {
                    var currentMonth = viewModel.getCurrentMonth_();
                    var newMonth = new Date(currentMonth.getTime());
                    newMonth.setFullYear(parseInt(year, 10));
                    var minMonth = _this.options_.getMinMonth_();
                    var maxMonth = _this.options_.getMaxMonth_();
                    if (newMonth.getTime() < minMonth.getTime()) newMonth = minMonth;
                    if (newMonth.getTime() > maxMonth.getTime()) newMonth = maxMonth;
                    if (!viewModel.goToMonth_(event, newMonth)) _this.yearActiveElement_.value = currentMonth.getFullYear() + "";
                }), this.options_);
            ...
            Template_.prototype.updateYearElement_ = function(viewModel) {
                if (!this.yearTextElement_) return;
                ...
                if (this.tableOfYearsElement_) {
                    ...
                    for (var index = 0; index < diff.remove.length; index++) this.yearActiveElement_.removeChild(diff.remove[index]);
                    for (index = diff.prepend.length - 1; index >= 0; index--) this.yearActiveElement_.insertBefore(TheDatepicker.HtmlHelper_.createSelectOption_(diff.prepend[index] + "", diff.prepend[index] + ""), this.yearActiveElement_.firstChild);
                    for (index = 0; index < diff.append.length; index++) this.yearActiveElement_.appendChild(TheDatepicker.HtmlHelper_.createSelectOption_(diff.append[index] + "", diff.append[index] + ""));
            ...
            Template_.prototype.createMonthAndYearElement_ = function(viewModel) {
                ...
                var selectElement = TheDatepicker.HtmlHelper_.createSelectInput_([], (function(event, value) {
                    ...
                    if (!viewModel.goToMonth_(event, newMonth)) _this.monthAndYearSelect_.value = _this.getMonthAndYearOptionValue_({
            ...
            Template_.prototype.updateMonthAndYearElement_ = function(viewModel) {
                var _this = this;
                if (!this.monthAndYearElement_) return;
                var currentMonth = viewModel.getCurrentMonth_();
                var currentData = {
                    month: currentMonth.getMonth(),
                    year: currentMonth.getFullYear()
                ...
                for (var index = 0; index < diff.remove.length; index++) this.monthAndYearSelect_.removeChild(diff.remove[index]);
                for (index = diff.prepend.length - 1; index >= 0; index--) {
                ...
                for (index = 0; index < diff.append.length; index++) {
                    data = this.getMonthAndYearByIndex_(diff.append[index]);
            ...
            Template_.prototype.calculateDropdownDiff_ = function(options, newRange, getNumerical) {
                ...
                var append = [];
                var remove = [];
                for (var value = newRange.from; value <= newRange.to; value++) if (firstOption === null || value < firstOption) prepend.push(value); else if (value > lastOption) append.push(value);
                for (var index = 0; index < options.length; index++) {
                    value = getNumerical(options[index].value);
                    if (value < newRange.from || value > newRange.to) remove.push(options[index]);
                }
                return {
                    prepend,
                    append,
            ...
            Template_.prototype.createTableHeaderCellElement_ = function(viewModel, dayOfWeek) {
                ...
                if (dayOfWeek === TheDatepicker.DayOfWeek.Saturday || dayOfWeek === TheDatepicker.DayOfWeek.Sunday) TheDatepicker.HtmlHelper_.addClass_(headerCell, TheDatepicker.ClassNameType.WeekDayWeekend, this.options_);
            ...
            Template_.prototype.createTableBodyElement_ = function(viewModel) {
                ...
                this.daysContentsElements_ = [];
                var rows = [];
                for (var index = 0; index < 6; index++) rows.push(this.createTableRowElement_(viewModel));
            ...
            Template_.prototype.updateWeeksElements_ = function(viewModel) {
                if (viewModel.yearSelectionState_) return;
                ...
                for (var weekIndex = 0; weekIndex < this.weeksElements_.length; weekIndex++) {
                    ...
                    if (week) for (var dayIndex = 0; dayIndex < this.daysElements_[weekIndex].length; dayIndex++) this.updateDayElement_(viewModel, this.daysElements_[weekIndex][dayIndex], this.daysButtonsElements_[weekIndex][dayIndex], this.daysContentsElements_[weekIndex][dayIndex], week[dayIndex]);
                }
            };
            ...
            Template_.prototype.updateDayElement_ = function(viewModel, dayElement, dayButtonElement, dayContentElement, day) {
                ...
                if (day.isToday) TheDatepicker.HtmlHelper_.addClass_(dayElement, TheDatepicker.ClassNameType.DayToday, this.options_);
                if (day.isPast) TheDatepicker.HtmlHelper_.addClass_(dayElement, TheDatepicker.ClassNameType.DayPast, this.options_);
                if (day.isWeekend) TheDatepicker.HtmlHelper_.addClass_(dayElement, TheDatepicker.ClassNameType.DayWeekend, this.options_);
                if (!day.isAvailable) {
                ...
                if (!day.isInCurrentMonth) TheDatepicker.HtmlHelper_.addClass_(dayElement, TheDatepicker.ClassNameType.DayOutside, this.options_);
                ...
                for (var index = 0; index < customClasses.length; index++) dayElement.className += " " + customClasses[index];
                dayButtonElement.style.visibility = "visible";
                if (day.isAvailable) dayButtonElement.href = "#"; else dayButtonElement.removeAttribute("href");
                if (day.isFocused) dayButtonElement.focus();
            };
            Template_.prototype.createTableCellButtonElement_ = function(viewModel) {
                ...
                var cellButton = TheDatepicker.HtmlHelper_.createAnchor_((function(event) {
                    ...
                    if (_this.options_.isHiddenOnSelect() && (isSelected || previous && cellButton.day.isEqualToDate(previous))) viewModel.close_(event);
                }), this.options_);
                ...
                cellButton.onmouseenter = function(event) {
                    if (_this.options_.getBeforeFocusListeners().length > 0 || _this.options_.getFocusListeners().length > 0) viewModel.highlightDay_(event || window.event, cellButton.day, false, false); else viewModel.cancelDayHighlight_(event || window.event);
                };
            ...
            Template_.prototype.createTableOfYearsBodyElement_ = function(viewModel) {
                ...
                this.yearsContentsElements_ = [];
                var rows = [];
                for (var index = 0; index < this.options_.getTableOfYearsRowsCount(); index++) rows.push(this.createTableOfYearsRowElement_(viewModel));
            ...
            Template_.prototype.updateTableOfYearsRowsElements_ = function(viewModel) {
                ...
                for (var rowIndex = 0; rowIndex < this.yearsElements_.length; rowIndex++) {
                    ...
                    if (cells) for (var columnIndex = 0; columnIndex < this.yearsElements_[rowIndex].length; columnIndex++) this.updateTableOfYearsCellElement_(viewModel, this.yearsElements_[rowIndex][columnIndex], this.yearsButtonsElements_[rowIndex][columnIndex], this.yearsContentsElements_[rowIndex][columnIndex], cells[columnIndex]);
                }
            };
            Template_.prototype.updateTableOfYearsCellElement_ = function(viewModel, yearElement, yearButtonElement, yearContentElement, yearCellData) {
                ...
                if (yearCellData.isAvailable) yearButtonElement.href = "#"; else {
                    yearButtonElement.removeAttribute("href");
                    if (this.options_.areYearsOutOfTableOfYearsVisible()) TheDatepicker.HtmlHelper_.addClass_(yearElement, TheDatepicker.ClassNameType.TableCellUnavailable, this.options_); else {
                        yearButtonElement.style.visibility = "hidden";
                        return;
                    }
                }
                if (yearCellData.isHighlighted) TheDatepicker.HtmlHelper_.addClass_(yearElement, TheDatepicker.ClassNameType.TableCellHighlighted, this.options_);
                if (yearCellData.isSelected) TheDatepicker.HtmlHelper_.addClass_(yearElement, TheDatepicker.ClassNameType.TableCellSelected, this.options_);
                yearButtonElement.style.visibility = "visible";
                if (yearCellData.isFocused) yearButtonElement.focus();
            };
            ...
            Template_.prototype.slideTable_ = function(viewModel, moveDirection, onComplete) {
                ...
                var trigger = function() {
                    ...
                    var onAfterSlide = function() {
                        if (_this.onAfterSlide_.length > 0) _this.onAfterSlide_.shift()(); else _this.onAfterSlide_ = null;
                    };
                    var listenerRemover;
                ...
                if (this.onAfterSlide_) this.onAfterSlide_.push(trigger); else {
                    this.onAfterSlide_ = [];
                    trigger();
                }
            };
            ...
            Template_.prototype.updateTitle_ = function(element, titleName) {
                ...
                if (title !== "") {
                    element.title = title;
                    if (this.options_.isAriaIncluded()) element.setAttribute("aria-label", title);
                } else {
    ...
    (function(TheDatepicker) {
        ...
        })(TheDatepicker.TitleName || (TheDatepicker.TitleName = {}));
        var Translator = function() {
            function Translator() {
                this.dayOfWeekFullTranslations_ = [ "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" ];
                this.dayOfWeekTranslations_ = [ "Su", "Mo", "Tu", "We", "Th", "Fr", "Sa" ];
                this.monthTranslations_ = [ "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" ];
                this.monthShortTranslations_ = [ "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" ];
                this.titles_ = [ "Go to previous month", "Go to next month", "Close calendar", "Reset calendar", "Deselect date", "Month selection", "Year selection", "Show earlier years", "Show later years" ];
            }
            Translator.prototype.clone = function() {
                var translator = new Translator;
                var index;
                for (index = 0; index < this.dayOfWeekFullTranslations_.length; index++) translator.dayOfWeekFullTranslations_ = this.dayOfWeekFullTranslations_.slice(0);
                for (index = 0; index < this.dayOfWeekTranslations_.length; index++) translator.dayOfWeekTranslations_ = this.dayOfWeekTranslations_.slice(0);
                for (index = 0; index < this.monthTranslations_.length; index++) translator.monthTranslations_ = this.monthTranslations_.slice(0);
                for (index = 0; index < this.monthShortTranslations_.length; index++) translator.monthShortTranslations_ = this.monthShortTranslations_.slice(0);
                for (index = 0; index < this.titles_.length; index++) translator.titles_ = this.titles_.slice(0);
                return translator;
            };
    ...
    (function(TheDatepicker) {
        ...
        var YearSelectionState = function() {
            ...
            YearSelectionState.prototype.shiftPage = function(shift) {
                if (!this.canShiftPage(shift)) return false;
                this.page += shift;
                return true;
            };
            ...
            YearSelectionState.prototype.highlightYear = function(year, doFocus) {
                ...
                if (!this || year === this.highlightedYear) return false;
                this.highlightedYear = year;
                if (doFocus) this.isHighlightedYearFocused = true;
                if (year < this.getFirstYear()) this.shiftPage(-1);
                if (year > this.getLastYear()) this.shiftPage(1);
                return true;
            };
            YearSelectionState.prototype.cancelHighlight = function() {
                if (!this.highlightedYear) return false;
                this.highlightedYear = null;
                return true;
            };
        ...
        var ViewModel_ = function() {
            ...
            ViewModel_.prototype.render_ = function() {
                if (this.datepicker_.isDestroyed() || this.selectPossibleDate_()) return;
                var correctMonth = this.options_.correctMonth(this.getCurrentMonth_());
                if (this.goToMonth_(null, correctMonth)) return;
                if (!this.tableOfYearsSettings_) this.tableOfYearsSettings_ = {
            ...
            ViewModel_.prototype.setActive_ = function(event, value) {
                if (this.active_ === value) return true;
                if (value && !this.triggerOnBeforeOpen_(event) || !value && !this.triggerOnBeforeClose_(event)) return false;
                this.active_ = value;
                if ((!value && !this.setYearSelectionActive_(false) || value) && this.options_.isHiddenOnBlur()) this.render_();
                if (value) this.triggerOnOpen_(event); else this.triggerOnClose_(event);
                return true;
            };
            ...
            ViewModel_.prototype.getCurrentMonth_ = function() {
                if (!this.currentMonth_) this.setCurrentMonth_(this.getInitialMonth_());
                return this.currentMonth_;
            };
            ViewModel_.prototype.canGoDirection_ = function(isForward) {
                var delta = isForward ? 1 : -1;
                if (this.yearSelectionState_) return this.yearSelectionState_.canShiftPage(delta);
            ...
            ViewModel_.prototype.canGoToMonth_ = function(month) {
                if (!TheDatepicker.Helper_.isValidDate_(month)) return false;
            ...
            ViewModel_.prototype.goDirection_ = function(event, isForward) {
                ...
                if (this.yearSelectionState_) {
                    if (this.yearSelectionState_.shiftPage(delta)) this.render_();
                    return;
                }
                var newMonth = new Date(this.getCurrentMonth_().getTime());
                newMonth.setMonth(newMonth.getMonth() + delta);
            ...
            ViewModel_.prototype.goToMonth_ = function(event, month, doCancelHighlight) {
                ...
                month.setDate(1);
                if (month.getTime() === this.getCurrentMonth_().getTime() || !this.canGoToMonth_(month)) return false;
                if (!this.triggerOnBeforeMonthChange_(event, month, this.currentMonth_)) return false;
                this.setCurrentMonth_(month);
                if (!doCancelHighlight || !this.cancelDayHighlight_(event)) this.render_();
                ...
                if (this.selectedDate_ && this.options_.isSelectedDateBoundWithMonth()) {
                    ...
                    if (date) this.selectDay_(event, date);
                }
                return true;
            };
            ...
            ViewModel_.prototype.selectDay_ = function(event, date, doUpdateMonth, doHighlight, canToggle) {
                ...
                if (!date) return this.cancelSelection_(event);
                var day;
                if (date instanceof TheDatepicker.Day) {
                    day = date;
                    date = day.getDate();
                } else day = this.createDay_(date);
                if (!day.isAvailable) return false;
                if (day.isEqualToDate(this.selectedDate_)) {
                    if (canToggle && this.options_.hasToggleSelection()) return this.cancelSelection_(event);
                    return false;
                }
                var previousDay = this.selectedDate_ ? this.createDay_(this.selectedDate_) : null;
                if (!this.triggerOnBeforeSelect_(event, day, previousDay)) return false;
                this.selectedDate_ = day.getDate();
                if (doHighlight) this.highlightDay_(event, day);
                doUpdateMonth = doUpdateMonth || this.options_.isSelectedDateBoundWithMonth();
                if (!doUpdateMonth || !this.goToMonth_(event, date)) this.render_();
            ...
            ViewModel_.prototype.selectPossibleDate_ = function() {
                try {
                    ...
                    if (!(error instanceof TheDatepicker.AvailableDateNotFoundException)) throw error;
            ...
            ViewModel_.prototype.highlightDay_ = function(event, day, doUpdateMonth, doFocus) {
                ...
                if (doFocus === void 0) doFocus = true;
                if (!day.isAvailable) return false;
                if (day.isEqualToDay(this.highlightedDay_)) return false;
                var previousDay = this.highlightedDay_;
                if (!this.triggerOnBeforeFocus_(event, day, previousDay)) return false;
                this.highlightedDay_ = day;
                if (doFocus) this.isHighlightedDayFocused_ = true;
                var date = day.getDate();
                if (!doUpdateMonth || !this.goToMonth_(event, date, false)) this.render_();
            ...
            ViewModel_.prototype.highlightFirstAvailableDay_ = function(event) {
                ...
                while (!day.isAvailable) {
                    ...
                    if (sibling.dayNumber === 1) break;
                    if (sibling.getDate().getTime() > maxDate.getTime()) break;
                    day = sibling;
                }
            ...
            ViewModel_.prototype.cancelSelection_ = function(event, force) {
                ...
                if (!this.options_.isAllowedEmpty() && !force) return false;
                if (!this.selectedDate_) return false;
                var previousDay = this.createDay_(this.selectedDate_);
                if (!this.triggerOnBeforeSelect_(event, null, previousDay)) return false;
                this.selectedDate_ = null;
                this.render_();
            ...
            ViewModel_.prototype.cancelDayHighlight_ = function(event) {
                if (!this.highlightedDay_) return false;
                var previousDay = this.highlightedDay_;
                if (!this.triggerOnBeforeFocus_(event, null, previousDay)) return false;
                this.highlightedDay_ = null;
                this.render_();
            ...
            ViewModel_.prototype.highlightSiblingYear_ = function(year, direction) {
                ...
                if (newYear < this.options_.getMinDate_().getFullYear() || newYear > this.options_.getMaxDate_().getFullYear()) return;
            ...
            ViewModel_.prototype.getWeekDays_ = function() {
                var weekDays = [];
                for (var day = 0; day < 7; day++) weekDays.push((this.options_.getFirstDayOfWeek() + day) % 7);
                return weekDays;
            };
            ViewModel_.prototype.getWeeks_ = function() {
                ...
                for (var date = 1; date <= monthDaysCount; date++) days.push(this.createDay_(new Date(currentMonth.getFullYear(), currentMonth.getMonth(), date)));
                for (index = 0; index < outsideDates.append.length; index++) {
                    day = this.createDay_(outsideDates.append[index]);
                    days.push(day);
                }
                var weeks = [];
            ...
            ViewModel_.prototype.getYearsRows_ = function() {
                ...
                for (var year = firstYear; year <= firstYear + this.yearSelectionState_.cellsCount; year++) {
                    ...
                    if (year > maxYear || year < minYear) yearCellData.isAvailable = false; else {
                        if (year === currentYear) yearCellData.isSelected = true;
                        if (year === this.yearSelectionState_.highlightedYear) {
                            yearCellData.isHighlighted = true;
                            if (this.yearSelectionState_.isHighlightedYearFocused) yearCellData.isFocused = true;
                        }
                    }
                    yearsData.push(yearCellData);
                }
                var yearsRows = [];
                for (var i = 0; i < yearsData.length; i += this.tableOfYearsSettings_.columnsCount) yearsRows.push(yearsData.slice(i, i + this.tableOfYearsSettings_.columnsCount));
            ...
            ViewModel_.prototype.createYearSelectionState_ = function() {
                ...
                if (align === TheDatepicker.Align.Left && minDate === null || align === TheDatepicker.Align.Right && maxDate === null) align = null;
                if (!align) if (minDate && maxDate) {
                ...
                } else if (minDate) align = TheDatepicker.Align.Left; else if (maxDate) align = TheDatepicker.Align.Right; else align = TheDatepicker.Align.Center;
                var lowestYear;
            ...
            ViewModel_.prototype.triggerKeyPress_ = function(event) {
                if (TheDatepicker.Helper_.inArray_([ TheDatepicker.KeyCode_.Left, TheDatepicker.KeyCode_.Up, TheDatepicker.KeyCode_.Right, TheDatepicker.KeyCode_.Down ], event.keyCode)) {
                    ...
                    if (this.yearSelectionState_) if (this.yearSelectionState_.highlightedYear) this.highlightSiblingYear_(this.yearSelectionState_.highlightedYear, moveDirection); else if (this.yearSelectionState_.getPage() === this.yearSelectionState_.initialPage) this.highlightSiblingYear_(this.getCurrentMonth_().getFullYear(), moveDirection); else this.highlightYear_(this.yearSelectionState_.getPage() === 0 ? this.yearSelectionState_.lowestYear : this.yearSelectionState_.getFirstYear()); else if (this.highlightedDay_) this.highlightSiblingDay_(event, this.highlightedDay_, moveDirection); else if (this.selectedDate_ && this.selectedDate_.getFullYear() === this.getCurrentMonth_().getFullYear() && this.selectedDate_.getMonth() === this.getCurrentMonth_().getMonth()) this.highlightSiblingDay_(event, this.createDay_(this.selectedDate_), moveDirection); else this.highlightFirstAvailableDay_(event);
                } else if (event.keyCode === TheDatepicker.KeyCode_.Esc && this.options_.isClosedOnEscPress()) if (this.yearSelectionState_) {
                    this.isYearSelectionToggleButtonFocused_ = true;
                    this.setYearSelectionActive_(false);
                } else this.datepicker_.close(event);
            };
            ViewModel_.prototype.createDay_ = function(date) {
                ...
                if (day.isInCurrentMonth) day.isVisible = true; else if (this.options_.areDaysOutOfMonthVisible()) {
                    var outsideDates = this.getOutsideDates_();
                    var pendants = outsideDates.prepend.concat(outsideDates.append);
                    for (var index = 0; index < pendants.length; index++) if (date.getTime() === pendants[index].getTime()) {
                        day.isVisible = true;
                        break;
                    }
                }
                if (day.isAvailable) {
                    if (day.isEqualToDate(this.selectedDate_)) day.isSelected = true;
                    if (day.isEqualToDay(this.highlightedDay_)) {
                        day.isHighlighted = true;
                        if (this.isHighlightedDayFocused_) day.isFocused = true;
                    }
                }
            ...
            ViewModel_.prototype.getOutsideDates_ = function() {
                ...
                var prepend = [];
                for (var date = lastMonthDaysCount - prependDaysCount + 1; date <= lastMonthDaysCount; date++) prepend.push(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1, date));
                var lastDateOfMonth = new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 0);
                var appendDaysCount = 6 - (lastDateOfMonth.getDay() - firstDayOfWeek + 7) % 7;
                var append = [];
                for (date = 1; date <= appendDaysCount; date++) append.push(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, date));
                if (this.options_.hasFixedRowsCount()) {
                    var monthDaysCount = lastDateOfMonth.getDate();
                    for (date = appendDaysCount + 1; prependDaysCount + monthDaysCount + append.length < 6 * 7; date++) append.push(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, date));
                }
                this.outsideDates_ = {
                    prepend,
                    append
            ...
            ViewModel_.prototype.getInitialMonth_ = function() {
                if (!this.initialMonth_) this.initialMonth_ = this.options_.getInitialMonth();
